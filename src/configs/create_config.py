...
def serialize_obj(obj, indicator_list=None):
    ...
    if indicator_list:
        for idx, ind in enumerate(indicator_list):
            if hasattr(obj, '__dict__') and ind in obj.__dict__.values():
                return {
                    "type": cls_name,
                    "indicator_ref_index": idx
                }
...
def config_to_json(config):
    ...
    return {
        ...
        "indicators": [serialize_obj(ind) for ind in indicators],

        "identify_entry": [serialize_obj(entry, indicators) for entry in config.identify_entry],
        "identify_exit": [serialize_obj(exit_, indicators) for exit_ in config.identify_exit],
...
def deserialize_obj(obj_def, indicators=None):
    if obj_def is None:
        return None
...
def create_config_from_json(json_data):
    ...
    identify_entry = [deserialize_obj(obj, indicators) for obj in json_data["identify_entry"]]
    identify_exit = [deserialize_obj(obj, indicators) for obj in json_data["identify_exit"]]
...
#     # 1. Build indicators
#     indicators = []
#     for ind in json_data["indicators"]:
#         cls = CLASS_MAP[ind["type"]]
#         obj = cls(*ind.get("args", []))
#         indicators.append(obj)

#     # 2. Helper to create objects that reference indicators
#     def build_obj(obj_def):
#         cls = CLASS_MAP[obj_def["type"]]
#         if "indicator_ref_index" in obj_def:
#             return cls(indicators[obj_def["indicator_ref_index"]])
#         return cls()

#     # 3. Build rest of the components
#     identify_entry = [build_obj(obj) for obj in json_data["identify_entry"]]
#     identify_exit = [build_obj(obj) for obj in json_data["identify_exit"]]
#     entry_conditions = [build_obj(obj) for obj in json_data.get("entry_trade_conditions", [])]
#     exit_conditions = [build_obj(obj) for obj in json_data.get("exit_trade_conditions", [])]
#     buy_strategy = build_obj(json_data["buy_strategy"])
#     sell_strategy = build_obj(json_data["sell_strategy"])
#     exit_strategy = build_obj(json_data["exit_strategy"])

#     # 4. Pass everything to your actual Config constructor
#     config = Config(
#         name = json_data["name"],
#         mode = json_data["mode"],
#         trade = json_data["trade"],
