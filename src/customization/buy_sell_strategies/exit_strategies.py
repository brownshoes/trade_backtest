...
class LimitExitPercentAbove:
    ...
    def initialize(self):
        self.percent_above_current_price = Decimal(self.percent_above_current_price)

    def create_sell_order(self, open_position, exg_state):
        order_number = exg_state.provide_order_number()
        quantity = open_position.buy_info.order.execution_coin_amount - open_position.amount_sold
        quantity = quantize(quantity)

        execution_market_price = open_position.buy_info.order.execution_market_price
